...
function deriveTurnFolds(input: {
  ...
  for (const [turnId, group] of groupsByTurnId) {
    ...
    for (const entry of group.entries) {
      ...
      // Check-watch and agent-spawn rows represent state that can outlive the
      // turn, so keep them visible after the rest of the turn folds.
      if (entry.kind === "work" && entry.entry.githubActions !== undefined) {
        continue;
      }
